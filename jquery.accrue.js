...
;(function( $, window, document, undefined ){
    ...
    function formatNumber (num, currencySymbol) {
        return (currencySymbol || '')  + num.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
    }
    ...
    var calculateBasic = function( elem, options, output_elem ){
        ...
        if ( loan_info!==0 ) {
            ...
            var output_content = options.response_basic
                .replace( "%payment_amount%", formatNumber(loan_info.payment_amount_formatted, options.currency_symbol) )
                .replace( "%num_payments%", loan_info.num_payments )
                .replace( "%total_payments%",formatNumber(loan_info.total_payments_formatted, options.currency_symbol) )
                .replace( "%total_interest%", formatNumber(loan_info.total_interest_formatted, options.currency_symbol) );
    ...
    var calculateComparison = function( elem, options, output_elem ){
        ...
        if ( loan_1_info!==0 && loan_2_info!==0 ) {
            ...
            var output_content = options.response_compare
                .replace( "%savings%", formatNumber(callback_data.savings.toFixed(2), options.currency_symbol) )
                .replace( "%loan_1_payment_amount%", formatNumber(loan_2_info.payment_amount_formatted, options.currency_symbol) )
                .replace( "%loan_1_num_payments%", loan_2_info.num_payments )
                .replace( "%loan_1_total_payments%", loan_2_info.total_payments_formatted )
                .replace( "%loan_1_total_interest%", formatNumber(loan_2_info.total_interest_formatted, options.currency_symbol) )
                .replace( "%loan_2_payment_amount%", formatNumber(loan_1_info.payment_amount_formatted, options.currency_symbol) )
                .replace( "%loan_2_num_payments%", loan_1_info.num_payments )
                .replace( "%loan_2_total_payments%", loan_1_info.total_payments_formatted )
                .replace( "%loan_2_total_interest%", formatNumber(loan_1_info.total_interest_formatted, options.currency_symbol) );
            output_elem.html( output_content );
        ...
        if ( loan_info!==0 ) {
            ...
            for ( var i=0; i<loan_info.num_payments; i++) { 
                ...
                output_content = output_content+ 
                    ...
                    '<'+cell_tag+' class="accrue-payment-amount">'+formatNumber(loan_info.payment_amount_formatted, options.currency_symbol)+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-total-interest">'+formatNumber(counter_interest.toFixed(2), options.currency_symbol)+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-total-payments">'+formatNumber(counter_payment.toFixed(2), options.currency_symbol)+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-balance">$'+formatNumber(counter_balance.toFixed(2), options.currency_symbol)+'</'+cell_tag+'>'+
                    '</tr>';
            }
